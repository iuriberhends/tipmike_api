...
CONSULTAS = {
    ...
    """,
    # Quantas questoes de PM-BA Soldado sustentam cada tier S.
    #
    # A regra do mentor_tiers e "achou 1 questao de Soldado -> tier S". Uma so
    # basta pra provar que caiu na prova, mas nao basta pra confiar: se essa
    # unica questao estiver mal classificada pelo Gran, um assunto inteiro vira
    # prioridade maxima por engano. Foi o que apareceu em 23/09 - "Crimes
    # Culposos II e III" eram tier S por causa de UMA questao que, lida, e de
    # ERRO DE TIPO. Esta consulta mede quanto do diagnostico esta nessa corda
    # bamba.
    "forca_do_s": """
        SELECT n.quantas,
               count(*) AS topicos
          FROM (
            SELECT t.id,
                   count(*) FILTER (
                     WHERE (q.orgao ~* 'PM ?BA|Pol[ií]cia Militar da Bahia'
                            OR q.orgao_sigla ~* 'PM ?BA|CBM ?BA')
                       AND q.cargo ~* 'soldado|pra[cç]a') AS quantas
              FROM mentor.topico t
              LEFT JOIN mentor.questao_topico qt ON qt.topico_id = t.id
              LEFT JOIN mentor.questao q ON q.id = qt.questao_id
             WHERE t.tier = 'S'
             GROUP BY t.id
          ) n
         GROUP BY n.quantas
    ...
    # problemas.
    # 'provas' = em quantas questoes de PM-BA Soldado o tier se apoia. Sem
    # isso a linha do alerta nao deixa distinguir um assunto que caiu 11 vezes
    # de outro que caiu 1 - e os dois apareciam iguais, como [S].
    "alerta": """
        SELECT t.assunto_ids,
               count(DISTINCT t.id) AS aulas,
               ...
               count(DISTINCT q.id) FILTER (
                   WHERE (q.orgao ~* 'PM ?BA|Pol[ií]cia Militar da Bahia'
                          OR q.orgao_sigla ~* 'PM ?BA|CBM ?BA')
                     AND q.cargo ~* 'soldado|pra[cç]a') AS provas
          FROM mentor.topico t
          JOIN mentor.suficiencia s ON s.topico_id = t.id
          LEFT JOIN mentor.questao_topico qt ON qt.topico_id = t.id
          LEFT JOIN mentor.questao q ON q.id = qt.questao_id
...
def main():
    ...
    # Quao firme e cada tier S. Tier S apoiado em 1 questao e verdadeiro
    # (caiu mesmo) mas fragil: se o Gran classificou essa unica questao num
    # assunto vizinho, o assunto inteiro vira prioridade maxima por engano.
    forca = consultar("forca_do_s")
    if forca:
        print("\n  --- FIRMEZA DOS TIER S (quantas questoes de Soldado) ---")
        frageis = 0
        for quantas, n in forca:
            selo = ""
            if quantas <= 1:
                selo = "  <- apoiado em 1 questao so"
                frageis += n
            print(f"     {quantas} questao(oes): {n:>3} assunto(s){selo}")
        if frageis:
            print(f"     {frageis} dos tier S dependem de uma unica questao.")
            print("     Confira essas com --evidencia antes de confiar na ordem.")
    ...
    print(f"\n  --- ALERTA VERMELHO (agrupado): {len(alertas)} assunto(s) ---")
    print("     [tier] assunto                          aulas  questoes  provas")
    for _, aulas, exemplo, n_uteis, motivo, tier, provas in alertas:
        selo = f"{aulas} aulas" if aulas > 1 else "1 aula"
        frouxo = "  (!) uma prova so" if (provas or 0) <= 1 else ""
        print(f"     [{tier}] {(exemplo or '')[:38]:38} {selo:8} "
              f"{n_uteis or 0:>3}q  {provas or 0:>2} prova(s){frouxo}")
