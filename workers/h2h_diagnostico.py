...
SELECT
    ...
    COUNT(*) FILTER (WHERE ABS(EXTRACT(EPOCH FROM (t.ts - c.ts))) <= 900) AS ate_15min
  FROM c JOIN t
    ON ((t.a = c.a AND t.b = c.b) OR (t.a = c.b AND t.b = c.a))
   AND ABS(EXTRACT(EPOCH FROM (t.ts - c.ts))) < $5
"""
...
async def diagnosticar(pool, *, casa: str, sport: str, nicks: list,
    ...
    por_jog = {}
    for r in linhas:
        por_jog.setdefault(r['jogador'], []).append(r['ts'])
    for n in nicks:
    ...
    n = int(cruz['n'] or 0)
    if n:
        ok_pl = _pct(cruz['placar_ok'], n)
        esp = _pct(cruz['espelhado'], n)
        prox = _pct(cruz['ate_15min'], n)
        checagens.append(_check(
            'Os placares conferem?', f'{ok_pl:.0f}%', ok_pl >= PISO_PLACAR,
            f'{_mil(cruz["placar_ok"])} de {_mil(n)} jogos vistos pelas duas '
            f'fontes têm o mesmo placar',
        ...
        checagens.append(_check(
            'Os horários das duas fontes batem?', 'sim' if prox >= 60 else 'não',
            prox >= 60,
            f'{prox:.0f}% dos jogos em comum aparecem no mesmo horário nas duas',
    ...
    fontes = [{'nome': NOME.get(r['fonte'], r['fonte']), 'jogos': int(r['jogos'] or 0),
               'ate': r['ate'].date().isoformat() if r['ate'] else None}
              for r in fontes_raw]
    return {
