...
def _indexar_placar_por_evento(ticks) -> dict:
    ...
    for evt, linhas in idx.items():
        try:
            ...
            # guarda so as MUDANCAS de placar: a busca pega o ultimo <= ts, entao
            # repetir o mesmo placar em 40 ticks seguidos nao muda resposta
            # nenhuma e so ocupa memoria (~10x menos linhas por evento).
            tss, placares = [], []
            ultimo = None
            for ts, sh, sa in linhas:
                if ultimo != (sh, sa):
                    tss.append(ts)
                    placares.append((sh, sa))
                    ultimo = (sh, sa)
            saida[evt] = (tss, placares)
        except Exception:
            continue
    return saida
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            # === FONTE = BANCO (comportamento atual) ===
                ...
                if torneios:
                    # v21 (21/ago): whitelist no NOME traduzido vs coluna liga
                    # com CODIGO — expandir nome->codigos (mesma regra do
                    # executor), senao backtest de bot com whitelist devolve
                    # 0 ticks. Blindado: erro na expansao segue so com nomes.
                    try:
                        from routers.torneios import expandir_torneios
                        _cods_t = [c.upper() for c in
                    ...
                    for t in torneios:
                        ...
                        n += 1
                    if _cods_t:
                        ors.append(f"upper(liga) = ANY(${n})")
                        params.append(_cods_t)
                        n += 1
                ...
                if torneios_excluir:
                    try:
                        from routers.torneios import expandir_torneios
                        _cods_e = [c.upper() for c in
                                   expandir_torneios(bot.get('casa'),
                                                     torneios_excluir)]
                    except Exception:
                        _cods_e = []
                    ands = []
                    for t in torneios_excluir:
                        ...
                        n += 1
                    if _cods_e:
                        ands.append(f"upper(liga) <> ALL(${n})")
                        params.append(_cods_e)
                        n += 1
        ...
        # So monta quando o job e de HANDICAP: nenhum outro mercado le esse
        # indice, e varrer a lista crua (milhoes de linhas em job grande) so
        # pra jogar fora custaria RAM e tempo a toa.
        _idx_placar: dict = {}
        if _mercado_eh_hc(bot.get('mercado', '')):
            try:
